...
df <- data.frame(date = apply(expand.grid(dates, c("00","01","02","03","04","05","06",
                 ...
                 remainder_mean = colSums(scrime::rowScales(remainder) > 10),
                 stringsAsFactors = F)
...
extreme <- rowSums(scrime::rowScales(remainder) > 10)
shp <- cbind(shp, extreme)

bks <- seq(0,40,8)
temperature <- c('#000000','#bb1909','#ffa100','#ffff67','#ffffff')

par(mar = c(0,0,1.5,0))
choroLayer(x = shp, var = "extreme", breaks = bks, col = temperature,
             border = "khaki", lwd = 0.001, 
             legend.title.txt = "", 
             legend.pos = 'bottomleft', legend.values.rnd = 0)
layoutLayer(title = "Number of hours\nwith unusually very\nhigh activity", 
              sources = "", north = TRUE, 
              scale = 50, tabtitle = TRUE,
              frame = T,  
              author = "*model based on mobile phone data")
...
pal <- colorNumeric(c("black", "red", "yellow", "white"), bks)
