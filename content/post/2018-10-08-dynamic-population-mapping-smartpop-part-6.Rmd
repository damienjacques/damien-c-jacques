...
Pop <- list(PopWorkingDays_WE_DiffRef_Mean, PopWeekHolidays_WE_DiffRef_Mean, PopWorkingDays_WE_DiffRef_CV, PopWeekHolidays_WE_DiffRef_CV)
...
for (i in 3:4) {
  ...
  # quantization breaks of the rate
  bks <- c(0,5,10,15,20, max(shp[[names(Pop)[i]]]))

  # get a color palette
  cols <- c('#f2f0f7','#cbc9e2','#9e9ac8','#756bb1','#54278f')
  
  choroLayer(x = shp, var = names(Pop)[i], breaks = bks, col = cols,
             border = "khaki", lwd = 0.001, 
             legend.title.txt = "Coefficient of variation (%)", 
             legend.pos = 'bottomleft', legend.values.rnd = 0)
  layoutLayer(title = paste0("Coefficient of variation of population density over a working week (%)"), 
              sources = "", north = TRUE, 
              scale = 50, tabtitle = TRUE,
              frame = T,  
              author = "*model based on mobile phone data")
