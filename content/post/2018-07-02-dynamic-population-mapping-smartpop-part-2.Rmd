...
The next step is to build a grid of reference that will be used to classify the land use and estimate the population. To do that, a new assumption is required. 

{{% alert warning %}}
Assumption 4: when an antenna is not active during the day, it means that during this period, its area is covered by the neighboring antennas (i.e. some voronoi polygons become larger when an antenna is not active). 
{{% /alert %}}
...
A reference grid (shapefile of voronoi polygons) is defined to have a spatially consistent database with values for each hour in each polygon of reference. All antennas remaining in the database are used to define this grid. 
...
We need to disaggregate the value of each hour in this grid. First let's compute the gride of each hour.
