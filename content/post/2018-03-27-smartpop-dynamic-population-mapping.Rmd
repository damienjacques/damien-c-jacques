...
crs(antenna_shp) <- CRS("+init=epsg:103300") # Lambert 72 proj !! NOT EQUAL to EPSG:31300
...
```{r}
cadastre_antenna <- readOGR("/home/ubuntu/Dropbox/Research/Projects/SMARTPOP/Data/Cadastre_antenna/antenna_orange_cadastre.shp")
...
leaflet() %>% 
  ...
  addCircles(data = antenna_latlon, ~lon, ~lat,  weight = 5,  
             color="green", fillOpacity = 0.8, group = "antenna db") %>% 
  addCircles(data = cadastre_latlon, ~lon, ~lat,  weight = 5,  
             color="red", fillOpacity = 0.8, group = "antenna cadastre") %>% 
  # Layers control
  addLayersControl(
    baseGroups = c("Toner", "OSM (default)", "Toner Lite"),
