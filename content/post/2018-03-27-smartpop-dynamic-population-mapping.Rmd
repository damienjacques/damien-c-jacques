...
{{% alert warning %}}
Assumption 2: Very close antennas are antennas from the same BTS OR the same BTS but active at different time of the year. These antennas will be merged. Close antennas (> 2 m, < 1 km) will also be merged but in a further step. Other points correspond to all antennas of one BTS.
{{% /alert %}}
...
The number of antenna increase over time (trend) with a daily pattern (day / night). For the trend, there is three different periods: before 100 first days (around the beginning of August), then from 100 to 315 (mid of March) and the rest. 
...
A [shiny app](https://smartpop.shinyapps.io/ExploreAntenna/) is developped to check each antenna with less than 365 active days (609), one by one and create a lookup table to merge the antennas depending on their time series.
...
After visual exploration several types of antenna can be identified:

* **Type 1**: antennas active all days with a regular pattern without apparent anomalies.
* **Type 2**: antennas active all days with an irregular pattern and potential anomalies. 
* **Type 3**: antennas that are set up for special events, active for a couple of days (for the duration of the event) with a peak in SIM numbers (summer festivals...).
* **Type 4**: antennas covering commercial area. Most probably inside the building as they show lower SIM numbers than antenna around.
* **Type 5**: antennas that complete each other time series without overlap. They look like the same antenna but with different location points.
* **Type 5 bis**: same as type 5 but with overlap in time.
* **Type 6**: antennas that have a regular patterns and appear at some point in time. They look like new antennas. With the new antenna, the number of SIM active in the antennas decrease as the users can connect to more antennas.
* **Type 7**: same as type 5 but without changed in activity of antennas around. It looks like an area that was previously poorly covered by the network and new users can access the network thanks to the new antenna. It concerns only few users otherwise some trends should be visible on the time series of total SIM over time.
* **Type 8**: antennas in events area/buildings (such as national stadium, spiroudome, Bruxelles expo) with high peak during irregular events.
* **Type 9**: antennas with SIM very close to 0 for each active days.

The lookup table build tahnks to the shiny app is used to merge the antennas.
```{r}
...
The final database is almost ready. Zero values are added to antenna with < 365 days of activity and located in event areas (e.g. Bruxelles expo) identified during the visual exloration using the shiny app.
...
{{% alert warning %}}
Assumption 3: there is *nobody* in areas covered by antennas located in "event areas"" (e.g. Bruxelles Expo) when these antenna are not active.
{{% /alert %}}

```{r, eval = F, echo = F}
fwrite(Mobile_Phone, file = "/home/ubuntu/Dropbox/Research/Projects/SMARTPOP/Data/CleanData/MobilePhone.csv")
writeOGR(antenna_shp, "/home/ubuntu/Dropbox/Research/Projects/SMARTPOP/Data/CleanData/antenna_shp.shp", layer = "antenna_shp", driver = "ESRI Shapefile")
```

The remainder of the analysis can be found [here](https://damien-c-jacques.rbind.io/post/dynamic-population-mapping-smartpop-part-2/).