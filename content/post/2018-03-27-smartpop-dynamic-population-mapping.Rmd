...
```{r}
UniqueByDate <- Mobile_Phone %>%                   
                 group_by(DATE_HOUR) %>%          
                 summarise(Unique_Elements = n_distinct(ID))  

plot(1:nrow(UniqueByDate), 
     UniqueByDate$Unique_Elements, 
     type = "l", 
...
The number of antenna increase over time (trend) with a daily pattern (day / night). For the trend, there is three different periods: before 100 first days (around the beginning of August), then from 100 to 315 (mid of March) and the rest. We will take the max number of antenna during the first period as reference and merge the new antenna that appear after with their closest antenna. The result is a loss in spatial resolution.
...
A shiny application is developped to check each antenna with less than 365 active days, one by one and create a lookup table to merge the antennas depending on their time series.
